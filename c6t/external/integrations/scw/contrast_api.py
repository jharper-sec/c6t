...
def contrast_instance_from_json(credentials: ContrastAPICredentials):
    return ContrastTeamServer(credentials.base_url, credentials.api_key, _create_authorization_header(credentials))
...
def _create_authorization_header(credentials: ContrastAPICredentials) -> str:
    ...
    unencoded_authorization_header = (
        f"{credentials.username}:{credentials.service_key}"
    )
